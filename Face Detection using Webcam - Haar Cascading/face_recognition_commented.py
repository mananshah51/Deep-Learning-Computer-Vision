...
def detect(gray, frame): # We create a function that takes as input the image in black and white (gray) and the original image (frame), and that will return the same image with the detector rectangles. 
    ...
    for (x, y, w, h) in faces: # For each detected face:
        cv2.rectangle(frame, (x, y), (x+w, y+h), (255, 255, 0), 2) # We paint a rectangle around the face.
        ...
        for (ex, ey, ew, eh) in eyes: # For each detected eye:
            cv2.rectangle(roi_color,(ex, ey),(ex+ew, ey+eh), (0, 255, 255), 4) # We paint a rectangle around the eyes, but inside the referential of the face.
...
while True: # We repeat infinitely (until break):
    ...
    cv2.imshow('Face Detection Output', canvas) # We display the outputs.
